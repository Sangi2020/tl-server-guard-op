...
const lightText = document.querySelector('.nav-brand .light-text');
const darkText = document.querySelector('.nav-brand .dark-text');

// Function to handle the scroll event
function handleScroll() {
  if (window.scrollY > 50) {  // When user scrolls down 50px or more
    lightText.style.display = 'none';  // Hide light text
    darkText.style.display = 'inline-block';  // Show dark text
  } else {
    lightText.style.display = 'inline-block';  // Show light text
    darkText.style.display = 'none';  // Hide dark text
  }
}
...
// Run the function once to set the initial state based on the page load
handleScroll();
